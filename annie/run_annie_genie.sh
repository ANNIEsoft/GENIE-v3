...
#Find file
export GENIEXSECFILE=gxspl-FNALsmall.xml
...
Run a GENIE job
   ...
   -f | --flux=FLXFILES    flux files [$FLXFILES]
   -x | --xsec=GENIEXSECFILE cross section file [$GENIEXSECFILE]
...
process_args() {
  ...
  TEMP=`getopt -n $0 -s bash -a \
     --longoptions="help outdir: flux: xsec: run: nevt: exposure: pots: seed: gdml: topvol: tune: with-mec genlist: xsec-qual: fid: expand: rockbox:: rock helpfid" \
     -o hvo:f:x:r:n:e:s:g:t:u:F:E:R:-: -- "$@" `
  eval set -- "${TEMP}"
  unset TEMP
  while [ $# -gt 0 ]; do
    ...
    case "$1" in
	  ...
	  -f | --flux       ) export FLXFILES="$2";  shift  ;;
	  -x | --xsec       ) export GENIEXSECFILE="$2"; shift ;;
  ...
  if [ -z "$FLXFILES" ]; then
    echo "$b0: you must set the flux files using the -f flag"
    usage_exit=127
  fi
  if [ -z "$RUNBASE" ]; then
...
if [ ! -f $GDMLBASE ]; then
  # no?
  echo "$b0: no local copy of geometry"
fi
if [ -f $GDMLBASE ]; then
...
export FLUX="${FLXFILES},${FLXPSET}"


# put it all together
XSEC="--cross-sections $GENIEXSECFILE"
...
echo " "
echo $EXE -r $RUN $SEEDFLG $TOPVOLFLG -f $FLUX -g $GDML $UNITS $XSEC $GENFLG $MAXPL $LIMIT $MSGLVL $FIDFLG $ZMINFLG $TUNEFLG
     $EXE -r $RUN $SEEDFLG $TOPVOLFLG -f $FLUX -g $GDML $UNITS $XSEC $GENFLG $MAXPL $LIMIT $MSGLVL $FIDFLG $ZMINFLG $TUNEFLG > gntp.${RUN}.ghep.log 2>&1
